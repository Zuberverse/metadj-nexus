...
export interface MetaDjAiMessage {
  ...
  /** Optional message kind for non-chat events (legacy mode + model markers) */
  kind?: 'mode-switch' | 'model-switch';
  /** Legacy mode metadata (no longer surfaced in UI) */
  mode?: MetaDjAiMode;
  sources?: Array<{
    title: string;
    url: string;
...
export interface MetaDjAiContext {
  ...
  /** Internal intent hint (adaptive default; legacy values ignored in UI) */
  mode?: MetaDjAiMode;
  pageContext?: {
