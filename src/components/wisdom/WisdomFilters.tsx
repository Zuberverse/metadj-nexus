      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex flex-col gap-3 sm:items-end">
          <div className="space-y-1.5">
            <p className="text-xs font-heading font-semibold uppercase tracking-wider text-white/70">Length</p>
            <select
              value={selectedLength}
              onChange={(event) => onLengthChange(event.target.value as ReadTimeFilter)}
              className="w-full rounded-xl border border-white/10 bg-black/60 px-4 py-2 text-sm font-heading font-medium text-white/90 focus-ring"
            >
          ...
          {onReset && (
            <button
              ...
              onClick={onReset}
              className={clsx(
                "rounded-full border px-4 py-2 text-xs font-heading font-semibold uppercase tracking-wider transition",
                isFiltered
