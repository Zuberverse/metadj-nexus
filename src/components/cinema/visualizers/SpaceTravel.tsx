...
const StarShader = {
  ...
  vertexShader: `
    ...
    void main() {
      ...
      float pulse = 1.0 + uBass * 0.08 * sin(pos.z * 0.02 + uTime * 0.5);
      pos.x *= pulse;
      pos.y *= pulse;
      
      vColorIdx = aColorIdx;
      vStretch = 1.0 + (uSpeed / 25.0) * 0.8;
      
      float twinkle = sin(uTime * aTwinkleSpeed + aTwinklePhase);
      twinkle = twinkle * 0.5 + 0.5;
      twinkle = pow(twinkle, 2.0);
      vTwinkle = twinkle * 0.7;

      vec4 mvPosition = modelViewMatrix * vec4(pos, 1.0);
      gl_Position = projectionMatrix * mvPosition;

      float dist = length(mvPosition.xyz);
      float baseSize = aSize * (1.0 + uHigh * 0.3);
      baseSize *= (1.0 + twinkle * 0.4);
...
const NebulaShader = {
  ...
  vertexShader: `
    ...
    void main() {
      ...
      float breathe = sin(uTime * 0.2 + aColorIdx * 10.0) * uMid * 3.0;
      pos.x += breathe;
      pos.y += breathe * 0.5;
...
export function SpaceTravel({ bassLevel, midLevel, highLevel, performanceMode = false }: SpaceTravelProps) {
  ...
  useFrame((state, delta) => {
    ...
    smoothedBass = THREE.MathUtils.lerp(smoothedBass, Math.pow(bassLevel, 1.5), 0.05)
    smoothedMid = THREE.MathUtils.lerp(smoothedMid, midLevel, 0.03)
    smoothedHigh = THREE.MathUtils.lerp(smoothedHigh, Math.pow(highLevel, 1.3), 0.045)
    ...
    const accelRate = targetSpeed > smoothedSpeed ? 0.035 : 0.008
    smoothedSpeed = THREE.MathUtils.lerp(smoothedSpeed, targetSpeed, accelRate)
    smoothedSpeed = Math.max(smoothedSpeed, 1.0)
    smoothedSpeed = Math.min(smoothedSpeed, 28.0)

    accumulatedZOffset += smoothedSpeed * clampedDelta
    
    const baseRotationSpeed = 0.015
    const energyRotation = audioEnergy * 0.008
    accumulatedRotation += (baseRotationSpeed + energyRotation) * clampedDelta
    
    const colorSpeed = 0.35 + smoothedBass * 1.0 + smoothedMid * 0.4 + smoothedHigh * 0.25
    ...
    // Nebula brightness burst system
    nebulaBurstTimer += clampedDelta
    if (nebulaBurstTimer >= nextBurstTime) {
      nebulaBurstIntensity = 1.0
      nebulaBurstTimer = 0
      nextBurstTime = NEBULA_BURST_INTERVAL_MIN + Math.random() * (NEBULA_BURST_INTERVAL_MAX - NEBULA_BURST_INTERVAL_MIN)
    }
    // Also trigger burst on strong bass hits
    if (smoothedBass > 0.7 && nebulaBurstIntensity < 0.3) {
      nebulaBurstIntensity = 0.8
    }
    // Clamp to prevent exceeding 1.0 from rapid triggers
    nebulaBurstIntensity = Math.min(1.0, nebulaBurstIntensity)
    // Decay burst intensity
    if (nebulaBurstIntensity > 0) {
      nebulaBurstIntensity = Math.max(0, nebulaBurstIntensity - clampedDelta * 0.8)
    }
    ...
    const driftAmount = 0.015 + (smoothedSpeed / 30.0) * 0.03
    ...
    state.camera.rotation.z = THREE.MathUtils.lerp(state.camera.rotation.z, drift, 0.01)
    state.camera.rotation.x = THREE.MathUtils.lerp(state.camera.rotation.x, tilt * 0.2, 0.008)
  })
